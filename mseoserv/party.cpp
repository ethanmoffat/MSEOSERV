...
/* $Id: party.cpp 490 2015-11-18 10:46:34Z sausage $
...
void Party::Join(Character *character)
{
	...
	builder.AddChar(util::clamp<int>(static_cast<const int>(double(character->hp) / double(character->maxhp) * 100.0), 0, 100));
...
void Party::RefreshMembers(Character *character, bool create)
{
	...
	UTIL_FOREACH(this->members, member)
	{
		...
		builder.AddChar(util::clamp<int>(static_cast<const int>(double(member->hp) / double(member->maxhp) * 100.0), 0, 100));
