...
/* $Id: npc.cpp 511 2016-01-01 20:33:41Z sausage $
...
NPC::NPC(Map *map, short id, unsigned char x, unsigned char y, unsigned char spawn_type, short spawn_time, unsigned char index, bool temporary)
{
	...
	this->marriage_timer = 0;
	this->marriage_state = 0;
	this->marriage_partner[0] = "";
	this->marriage_partner[1] = "";
...
{
	return this->map->Walk(this, direction) != Map::WalkResult::WalkFail;
}
...
void NPC::Damage(Character *from, int amount, int spell_id)
{
	...
	if (this->hp > 0)
	{
		...
		builder.AddShort(util::clamp<int>(static_cast<int>(double(this->hp) / double(this->ENF().hp) * 100.0), 0, 100));
...
void npc_wedding(void *npc_void)
{
